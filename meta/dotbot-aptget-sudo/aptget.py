...
class AptGet(dotbot.Plugin):
    ...
    def handle(self, directive, data):
        if directive != self._directive:
            raise ValueError("AptGet cannot handle directive %s" % directive)
        return self._process_packages(data)

    def _process_packages(self, packages):
        self._context.defaults().get("aptget", {})
        results = {}
        ...
        for pkg_name in cleaned_packages["packages"]:
            ...
            if result not in successful:
                self._log.error(f"Could not install package '{pkg_name}'")
    ...
    def _install(self, pkg, commandPrefix):
        ...
        for item, values in self._strings.items():
            for text in values:
                index = out.find(text)
                if index >= 0:
                    return item
